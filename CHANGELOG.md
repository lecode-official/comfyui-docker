...
- Updated the base image of the Dockerfile to the latest version of PyTorch (from PyTorch 2.9.1, CUDA 12.8, and cuDNN 9 to PyTorch 2.11.0, CUDA 12.8, and cuDNN 9).
- Updated the ComfyUI version to the latest version (from ComfyUI 0.8.2 to ComfyUI 0.19.0).
- Updated the ComfyUI Manager version to the latest version (from ComfyUI Manager 4.0.5 to ComfyUI Manager 4.1).
- Updated the CI/CD workflow to use the latest versions of the Docker login action, Docker build-push action, and attest build provenance action.
- The copyright year in the license file was updated to 2026.
- The `libgl1-mesa-glx` package was a transitional package and is now obsolete from Ubuntu 23.10 and onwards. Instead, the `libgl1` and `libglx-mesa0` packages must now be installed to provide the `libGL.so.1` library, which is required by some custom nodes in ComfyUI. The Dockerfile was updated to reflect this change.
- The Open Container Initiative (OCI) version label automatically generated by the Docker Metadata Action was incorrect, so it is now set manually based on the name of the tag that was created for the release.
- The base image now uses an externally managed Python environment that no longer allows system-wide package installations via `pip`. To address this, a virtual environment is now created during the image build and used to install all Python dependencies. The entrypoint script was updated to activate the virtual environment before installing custom node requirements and starting ComfyUI.
